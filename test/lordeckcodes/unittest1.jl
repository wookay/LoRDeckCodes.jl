...
file_string = replace(read_remove_bom(normpath(@__DIR__, "DeckCodesTestData.txt")), "\r" => "")
deck_blocks = split(file_string, "\n\n")
for deck_block in deck_blocks
    ...
    card_list = filter(x -> !isempty(x),deckcode_and_card_list[2:end])
